...
interface ButtonProps {
  ...
  topText: string;
  bottomText: string;
  width?: string; // For Tailwind class, e.g., "w-52"
  height?: string; // For Tailwind class, e.g., "h-24"
}
...
const ButtonComponent: React.FC<ButtonProps> = ({
  ...
  topText,
  bottomText,
  width = "w-full", // Default to w-full for responsiveness
  height = "h-auto", // Default to h-auto
}) => (
  <button
    className={`px-3 py-2 rounded-lg ${width} ${height} bg-primary/60 text-white text-sm hover:shadow-[4px_4px_0px_0px_rgba(2,7,33)] transition duration-200 flex justify-center md:justify-start items-center`}
  >
...
export function TailwindcssButtons({ width, height }: TailwindcssButtonsProps) {
  return (
    <div className="">
      <div className="flex w-full justify-center gap-3 md:flex-row flex-col items-center">
        {buttons.map((button, idx) => (
          ...
          >
            <ButtonsCard>{button.component(width, height)}</ButtonsCard>
          </a>
        ))}
      </div>
    </div>
...
export const buttons = [
  {
    ...
    component: (width?: string, height?: string) => (
      <ButtonComponent
